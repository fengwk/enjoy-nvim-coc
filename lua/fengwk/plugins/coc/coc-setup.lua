...
keyset("n", "g<leader>", "<Plug>(coc-implementation)", { silent = true })
...
-- keyset("n", "<leader>fm", "<Plug>(coc-format-selected)", { silent = true }) -- TODO 未生效
...
keyset("n", "<leader>[E", "<Plug>(coc-fix-current)", opts)
keyset("n", "<leader>]E", "<Plug>(coc-fix-current)", opts)
...
keyset("n", "<leader>fm", "<Cmd>Format<CR>", { silent = true })

-- " Add `:Fold` command to fold current buffer.
vim.api.nvim_create_user_command("Fold", "call CocAction('fold', <f-args>)", { nargs = '?' }) -- TODO 未知用途

-- Add `:OR` command for organize imports of the current buffer.
vim.api.nvim_create_user_command("OR", "call CocActionAsync('runCommand', 'editor.action.organizeImport')", {})
keyset("n", "<leader>ip", "<Cmd>OR<CR>", { silent = true }) -- TODO 测试导入引用

-- Add (Neo)Vim's native statusline support.
-- NOTE: Please see `:h coc-status` for integrations with external plugins that
-- provide custom statusline: lightline.vim, vim-airline.
vim.opt.statusline:prepend("%{coc#status()}%{get(b:,'coc_current_function','')}") -- TODO 使用命令行扩展替换
...
keyset("n", "gw", ":<C-u>CocList -I symbols<cr>", opts)
-- Do default action for next item.
keyset("n", "[c", ":<C-u>CocNext<cr>", opts)
-- Do default action for previous item.
keyset("n", "]c", ":<C-u>CocPrev<cr>", opts)
-- Resume latest coc list.
keyset("n", "gW", ":<C-u>CocListResume<cr>", opts)
